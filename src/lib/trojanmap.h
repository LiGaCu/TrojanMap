...
class TrojanMap {
 ...
 private: 
  bool getNodebyNamePopulated;
  std::unordered_map<std::string, Node> getNodebyNameMap;
  std::vector<std::string> circle_ids;

  // To check if word begins with key
  bool WordBeginWith(const std::string &word, const std::string &key);

  // The helper function for TravellingTrojan
